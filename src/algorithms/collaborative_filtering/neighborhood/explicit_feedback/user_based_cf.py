...
class UserBasedExplicitCF(NeighborhoodUserCF):
    ...
    def _init_similarity(self, user_id, another_user_id):
        ...
        sim = pearson_correlation_terms(self.model[CO_RATED_KEY][(user_id,another_user_id)], user, another_user, self.model[AVG_RATINGS_KEY][user_id], self.model[AVG_RATINGS_KEY][another_user_id])
        self._update_similarities(user_id, another_user_id, sim[0], sim[1], sim[2], sim[3])
