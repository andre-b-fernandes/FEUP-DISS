...
class NeighborhoodUserCF(CollaborativeFiltering):
    ...
    def _init_similarities(self):
        self.model[SIMILARITIES_KEY] = SymmetricMatrix(len(self.matrix), self.similarity_data_structure())
    ...
    def _init_neighborhood(self):
        self.model[NEIGHBORS_KEY] = [ self._neighborhood(user_id) for user_id in range(0, len(self.matrix))] 
    ...
    #TODO Investigate better heuristics or approaches(Local Sensitive hashing?) than this standard knn.
