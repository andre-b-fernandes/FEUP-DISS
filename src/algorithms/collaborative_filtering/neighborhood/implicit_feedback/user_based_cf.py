...
class UserBasedImplicitCF(NeighborhoodUserCF):
    ...
    def _init_similarity(self, user_id, another_user_id):
        ...
        number_rated_items_another_user = len(self.co_rated_between(another_user_id, another_user_id))
        number_of_co_rated_items = len(self.co_rated_between(user_id, another_user_id))
        self.model[SIMILARITIES_KEY][(user_id,another_user_id)] = cosine_similarity(number_of_co_rated_items, number_rated_items_user, number_rated_items_another_user)
